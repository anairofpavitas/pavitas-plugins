---
name: update
description: Sync tasks and refresh memory from your current activity. Use when pulling new assignments from Notion into TASKS.md, triaging stale or overdue tasks, filling memory gaps for unknown people or projects, or running a comprehensive scan to catch todos buried in email.
...
- **Notion Tasks database** (project tracker + knowledge base for Pavitas)
...
```
Update complete:
- Tasks: +3 from Notion, 1 completed, 2 triaged
...
- **Email (Gmail):** Search sent messages
- **Documents (Notion):** List recently touched pages
- **Calendar (Google Calendar):** List recent + upcoming events
- **Zo memory:** Query Zo's own memory (`bun /home/workspace/.zo/memory/scripts/memory.ts hybrid "query"` via Zo's `bash` tool) for recent activity Zo has already picked up independently of this session

(No chat tool is connected for Pavitas — skip that source.)
...
- Check Supermemory before asking Pavi to explain a person/project/term that's decoded here for the first time — only ask if it's genuinely not already recorded there
- If Pavi wants this update running on a schedule, set it up as a Zo automation (`create_automation`) rather than a session-local scheduled task, so it keeps running even when this session isn't open
