...
You: "What did we decide about the API redesign?"
              ↓ Claude searches
Slack/Discord: #engineering thread from Tuesday with the decision
Gmail: Follow-up email from Sarah with the spec
Google Drive/Box: Updated API design doc (modified yesterday)
              ↓ Claude synthesizes
...
|--------|---------------|
| **Slack/Discord** | Messages, threads, channels, DMs |
| **Gmail** | Emails, attachments, conversations |
| **Google Drive/Box** | Docs, sheets, slides, PDFs |
...
Claude searches:
  Slack/Discord → #engineering, #infrastructure for "postgres" "switch" "decision"
  Gmail → threads with "postgres" in subject
  Google Drive/Box → docs mentioning database migration
...
Claude scans:
  Slack/Discord → channels you're in, DMs, mentions
  Gmail → inbox activity
  Google Drive/Box → docs shared with you or modified
...
Claude searches:
  Slack/Discord → messages about Kubernetes, k8s, clusters
  Google Drive/Box → docs authored about infrastructure
...
The more sources you connect via MCP, the more complete your search results. Start with Slack/Discord, Gmail, and Google Drive/Box, then add your wiki, project management tool, and CRM as needed.
